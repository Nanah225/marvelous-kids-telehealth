...
// Create video_consultations table if not exists
mysqli_query($conn, "CREATE TABLE IF NOT EXISTS video_consultations (
...
// Create doctors table if not exists with all required columns
...
$columns_to_add = [
    'is_available' => "ALTER TABLE doctors ADD COLUMN is_available TINYINT DEFAULT 1",
    'consultation_fee' => "ALTER TABLE doctors ADD COLUMN consultation_fee DECIMAL(10,2) DEFAULT 500.00",
    'experience_years' => "ALTER TABLE doctors ADD COLUMN experience_years INT DEFAULT 5",
    'profile_image' => "ALTER TABLE doctors ADD COLUMN profile_image VARCHAR(255) NULL"
];

foreach($columns_to_add as $column => $sql){
    $check = mysqli_query($conn, "SHOW COLUMNS FROM doctors LIKE '$column'");
...
$check_doctors = mysqli_query($conn, "SELECT COUNT(*) as cnt FROM doctors");
$doctor_count = mysqli_fetch_assoc($check_doctors);
if($doctor_count['cnt'] == 0){
    $doctors_data = [
        ['Dr. Sarah Johnson', 'Pediatrician - General Medicine', 12, 500],
        ['Dr. Michael Chen', 'Pediatric Pulmonologist', 8, 750],
        ['Dr. Emily Rodriguez', 'Pediatric Dermatologist', 10, 650],
        ['Dr. James Wilson', 'Pediatric Gastroenterologist', 15, 700],
        ['Dr. Lisa Anderson', 'Pediatric Neurologist', 11, 800]
    ];
    foreach($doctors_data as $doc){
        mysqli_query($conn, "INSERT INTO doctors (name, specialty, experience_years, consultation_fee, is_available) VALUES ('$doc[0]', '$doc[1]', $doc[2], $doc[3], 1)");
    }
}
